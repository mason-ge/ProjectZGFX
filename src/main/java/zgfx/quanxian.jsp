...
<script type="text/javascript">
    jQuery().ready(function () {
        ...
        if(owner_value==userid){
            //隐藏新增，删除，复制明细按钮
           /* jQuery("#addbutton1").show();
            jQuery("#delbutton1").show();
            jQuery("#copybutton1").show();*/
            flag="1"
            ...
            if(""!=allrowindex){
                ...
                for(var i=0;i<ids.length;i++){
                    var wjzrr_value=ModeForm.getFieldValue(wjzrr+"_"+ids[i]);
        ...
        if(xmfzr_value==userid ){
            ...
            allrowindex=  ModeForm.getDetailAllRowIndexStr("detail_1");
            if(""!=allrowindex){
                var ids=allrowindex.split(",");
                for(var i=0;i<ids.length;i++){
        ...
        //项目负责人 && 文件负责人
        if(wjzrr_value == userid && xmfzr_value==userid){
            flag="1"
            ModeForm.changeFieldAttr(xmfzrsfywh, 2);
            if(""!=allrowindex){
                var array=allrowindex.split(",");
                for (let i = 0; i <array.length ; i++) {
                    var wjzrr_value=ModeForm.getFieldValue(wjzrr+"_"+array[i]);
                    ModeForm.changeFieldAttr(wjzrr+"_"+array[i], 2);
                    ModeForm.changeFieldAttr(wjmc+"_"+array[i], 2);
                    ModeForm.changeFieldAttr(zwscrq+"_"+array[i], 2);
                    ModeForm.changeFieldAttr(wjlj+"_"+array[i], 2);
                    ModeForm.changeFieldAttr(sjscrq+"_"+array[i], 2);
                    ModeForm.changeFieldAttr(zt+"_"+array[i], 2);
                }
            }
        ...
        //owner && 项目负责人
        if(xmfzr_value==userid && owner_value==userid){
            flag="1"
            ModeForm.changeFieldAttr(xmmc, 2);
            ModeForm.changeFieldAttr(xmbh, 2);
            ModeForm.changeFieldAttr(lx, 2);
